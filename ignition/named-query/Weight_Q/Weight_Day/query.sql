...
SELECT 
    ...
    (SUM((a.pct_out_of_range / 100) * [total_count]) / SUM([total_count])) * 100 AS 'pct_out_of_range', 
  --  SUM(a.standard_deviation * [count]) / SUM([count]) AS 'standard_deviation',
    case when SUM([count]) > 0 then (SUM(a.standard_deviation * [count]) / SUM([count])) else 0 end AS 'standard_deviation',
  ...
  --  SUM(a.percentile25 * [count]) / SUM([count]) AS 'percentile25',
   ...
   -- SUM(a.percentile75 * [count]) / SUM([count]) AS 'percentile75',
    case when SUM([count]) > 0 then (SUM(a.percentile25 * [count]) / SUM([count])) else 0 end AS 'percentile25',
    case when SUM([count]) > 0 then (SUM(a.percentile50 * [count]) / SUM([count])) else 0 end AS 'percentile50',
    case when SUM([count]) > 0 then ( SUM(a.percentile75 * [count]) / SUM([count])) else 0 end AS 'percentile75',
