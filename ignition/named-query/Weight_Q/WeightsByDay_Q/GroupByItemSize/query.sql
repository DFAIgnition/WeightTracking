...
;WITH localised_aggregated AS (
	...
	and total_count > 0
			...
			SELECT
				...
				ISNULL(CAST(m.item_size AS VARCHAR), 'Unknown:' + trim(str(m.material_number))) as 'item_size'
			...
			FROM localised_aggregated as a
				...
				LEFT JOIN system.dbo.materials m on (TRY_CAST(a.material AS INT) = m.material_number)
			WHERE ((0 = :scale_id and f.is_line_total = 0) or a.scale_id = :scale_id)
			AND f.line_id = :line_id
			GROUP BY
			    a.[date], 
			    a.display_date  ,
			    ISNULL(CAST(m.item_size AS VARCHAR), 'Unknown:' + trim(str(m.material_number)))
			ORDER BY a.date desc, ISNULL(CAST(m.item_size AS VARCHAR), 'Unknown:' + trim(str(m.material_number)))